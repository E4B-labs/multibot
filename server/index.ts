    ...
    if (path.startsWith("/api/internal/")) {
      ...
      if (method === "POST" && path === "/api/internal/agent-action") {
        ...
        switch (action) {
          ...
          case "routines.update": { requireFull(); const routine = harnessRoutines.update(fromBotId, String(body.id), body); broadcast({ kind: "workspace", botId: fromBotId, resource: "routines" }); return json(res, routine ? 200 : 404, routine ? routineView(fromBotId, routine) : { error: "no such routine" }); }
