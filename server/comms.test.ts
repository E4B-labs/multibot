...
describe("mentionedBots", () => {
  ...
  it("matches a tag at a word start, case-insensitively", () => {
    expect(mentionedBots("hey @Research, look", peers).map((b) => b.id)).toEqual(["3"]);
    expect(mentionedBots("@Research first thing", peers).map((b) => b.id)).toEqual(["3"]);
  });
  ...
  it("dedupes repeats and collects multiple bots", () => {
    expect(mentionedBots("@Research and @New Bot and @Research", peers).map((b) => b.id)).toEqual(["3", "1"]);
  });
  it("ignores emails, hidden bots, and mid-word @", () => {
    expect(mentionedBots("mail user@example.com please", peers)).toEqual([]);
