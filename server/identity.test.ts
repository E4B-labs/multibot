...
describe("protocol v2 identity", () => {
  it("configures server, creates owner, rotates recovery and revokes access", async () => {
    ...
    expect(store.actorForRequest({ headers: { authorization: `Bearer ${registered.accessToken}` } })?.username).toBe("kacper");

    const recovered = await store.recover("kacper", registered.recoveryCode, "new-profile-password-123", "recovery");
    ...
    expect((await store.login("kacper", "new-profile-password-123", "new device")).actor.userId).toBe(registered.actor.userId);
    store.close();
  });
