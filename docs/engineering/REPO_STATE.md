...
| Testy | vitest: 105 plików / 640 testów (liczby zmierzone, nie policzone z drzewa) |
...
- **Wielkie pliki jako punkty kolizji**: `server/index.ts` 4735 linii,
  ...
  `src/state/store.tsx` 1119. Dwie gałęzie w `server/index.ts` naraz to
  najbardziej prawdopodobny konflikt w tym repo.
...
1. **Zrobione w tym PR**: `AGENTS.md` jako kanon, `docs/engineering/`, szablon
   ...
   `.dockerignore` z `.env*`, naprawa czerwonego CI, likwidacja sprzecznych
   list bramek.
