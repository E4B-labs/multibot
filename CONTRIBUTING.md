...
Data lives in `~/.openmausbot/` (legacy-compatible path: bots, transcripts, per-thread NDJSON event logs,
config with keys). Do not rename or commit this directory; existing users depend on its migration path.
