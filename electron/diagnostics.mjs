...
const JSON_KEY_VALUE_PAIR =
  /(["'])([A-Za-z0-9_.-]*(?:api[_-]?key|apikey|secret|token|password|passwd|authorization|auth[_-]?token|access[_-]?key|private[_-]?key)s?)\1\s*:\s*\1([^"']*)\1/gi;
const AUTHORIZATION =
...
export function redactSecretsInLine(line) {
  ...
  out = out.replace(JSON_KEY_VALUE_PAIR, (_match, quote, key, value) => `${quote}${key}${quote}:${quote}${mask(value)}${quote}`);
...
export function buildDiagnosticsReport({
  ...
  const lines = [];
  lines.push("MultiBot Desktop diagnostics");
...
export function diagnosticsFileName(date = new Date()) {
  ...
  return (
    `multibot-desktop-diagnostics-${date.getFullYear()}${pad(date.getMonth() + 1)}${pad(date.getDate())}` +
