...
describe("buildDiagnosticsReport", () => {
  ...
  it("never includes an absolute log path in the report heading", () => {
    const report = buildDiagnosticsReport({
      ...
      logPath: "/Users/example/Library/Logs/MultiBot/server.log",
    });
  ...
  it("masks generic key=value secrets and content-shaped tokens in the log tail", () => {
    const report = buildDiagnosticsReport({
      ...
      configSummary: {},
      logTail: [
        'config {"apiKey":"example-provider-key"}',
        "Authorization: Bearer abcdefghijklmnop",
        "password=hunter2000",
      ].join("\n"),
    });
    expect(report).not.toContain("example-provider-key");
...
describe("diagnosticsFileName", () => {
  it("uses multibot-desktop-diagnostics-YYYYMMDD-HHmmss.txt", () => {
    expect(diagnosticsFileName(new Date(2026, 7, 22, 16, 5, 9))).toBe(
      "multibot-desktop-diagnostics-20260822-160509.txt",
    );
  });
});
